...
/** SSR-safe snapshot — matches server render before localStorage is read. */
export function getDefaultMvpSnapshot(): Pick<Persisted, "products" | "orders"> {
  const { products, orders } = emptyPersisted()
